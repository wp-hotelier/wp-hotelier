...
class HTL_Email_Guest_Invoice extends HTL_Email {
	...
	function __construct() {
		...
		$this->title            = esc_html__( 'Guest invoice', 'hotelier' );

		$this->heading          = htl_get_option( 'emails_guest_invoice_heading', __( 'Invoice for reservation #{reservation_number}', 'hotelier' ) );
		$this->subject          = htl_get_option( 'emails_guest_invoice_subject', __( '{site_title} - Cancelled reservation #{reservation_number}', 'hotelier' ) );
