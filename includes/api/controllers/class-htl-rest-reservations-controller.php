...
class HTL_REST_Reservations_Controller extends HTL_REST_Controller {
	...
	public function prepare_item_for_response( $post, $request ) {
		...
		$data = array(
			...
			'subtotal'             => absint( $reservation->get_subtotal() ),
			'tax_total'            => absint( $reservation->get_tax_total() ),
			'discount_total'       => absint( $reservation->get_discount_total() ),
			'total'                => absint( $reservation->get_total() ),
			'deposit'              => absint( $reservation->get_deposit() ),
			'paid_deposit'         => absint( $reservation->get_paid_deposit() ),
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				...
				'balance_due'          => array(
					'description' => __( 'Balance due (integer).', 'wp-hotelier' ),
					'type'        => 'integer',
					'context'     => array( 'view' ),
				),
				'items'                => array(
