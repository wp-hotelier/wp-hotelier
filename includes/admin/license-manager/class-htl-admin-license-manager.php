...
class HTL_License_Manager {
	...
	public function activate_license() {
		...
		$response = wp_remote_post(
			$this->api_url,
			array(
				'user-agent' => 'Hotelier License Manager',
				'timeout'    => 15,
				'sslverify'  => false,
				'body'       => $api_params
			)
	...
	public function notices() {
		...
		if ( is_object( $license ) && 'valid' !== $license->license && empty( $showed_invalid_message ) ) {
			if ( empty( $_GET[ 'tab' ] ) || 'licenses' !== $_GET[ 'tab' ] ) {
				$messages[] = sprintf(
					__( 'You have invalid or expired license keys for WP Hotelier. Please go to the <a href="%s">Licenses page</a> to correct this issue.', 'hotelier' ),
